...
MoltenVK, bounded GeoBEACON reduced median measured cluster-plus-lighting GPU time from 0.3086 ms for
fixed LOD1/fixed clusters to 0.2621 ms in this 100-light validation workload; the result is reported
...
Every policy uses a separately streamed maximum-LOD diffuse reference. The saved PPM captures verify
that the compared buffers contain rendered city geometry rather than only a clear color. Mean MSE
was $3.09\times10^{-4}$ for fixed LOD1 and approximately $5.11\times10^{-4}$ for semantic adaptive
policies on MoltenVK. Geometry allocation dominates these values in the pilot, while the conservative
omitted-light bound remains a separate reported quantity.
